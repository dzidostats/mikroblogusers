    ...
    for _ in range(3):  # retry max 3x
        try:
            ...
            if resp.status_code == 200:
                return {"username": username, "data": resp.json()}
            else:
                sleep(1)
...
# Zapisz dane skompresowane gzip
output_path = args.output + ".gz"
with gzip.open(output_path, "wt", encoding="utf-8") as f:
    json.dump(results, f, ensure_ascii=False)
print(f"Saved {len(results)} users → {output_path}")
